;var formateMain = function () {};
formateMain.prototype = {
  /**
   * 处于20150101格式时间串
   * @param {type} date
   * @returns {undefined}
   */
  dateFormatByString: function (date) {
    return (date.substr(0, 4)) + "-" + (date.substr(4, 2)) + "-" + (date.substr(6, 2));
  },
  /**
   * 转换时间对象为星期几
   * @param date：js Date对象
   * @return 星期几
   */
  weekFormat: function (date) {
    var week = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"];
    //console.log(date);
    if (typeof (date) == 'string') {
      date = new Date(Number(date.substr(0, 4)), Number(date.substr(4, 2)), Number(date.substr(6, 2)))
    }
    //console.log(date);
    return week[date.getDay()];
  },
  weekFormats: function (date) {
    var week = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"];
    //console.log(date);
    if (typeof (date) == 'string') {
      date = new Date(Date.parse(date));
    }
    //console.log(date);
    return week[date.getDay()];
  },
  dateFormat: function (d, type, f) {
    /**
     * d: 两种类型可传；1，new Date()对象；2，'20151101'，字符串；
     * type: 返回值类型，0为当前日期，-1为当前日期前一天，1为当前日期后一天；注：type值范围为正负28之间；
     * f: 返回值格式，'YY-MM-DD'为年-月-日，'MM-DD'为月-日；
     */
    // Normalize the d param
    var result;
    if (typeof(d) == 'object' && d instanceof(Date)) {
      result = new Date(d.getTime());
    } else if (typeof (d) == 'string' || typeof (d) == 'number') {
      d = String(d);
      result = new Date(Number(d.substr(0, 4)), Number(d.substr(4, 2)) - 1, Number(d.substr(6)));
    } else if (!d) {
      result = new Date();
    } else {
      throw new Error('Invalid param d for dateFormat(d, type, f)');
    }

    // Caculate with the type param
    result.setDate(result.getDate() + parseInt(type));

    // Format return value with format specified by the f param
    var yy = result.getFullYear(),
      mm = result.getMonth() + 1,
      dd = result.getDate();
    //保证yy mm dd 占两位字符
    mm = mm < 10 ? '0' + mm : mm;
    dd = dd < 10 ? '0' + dd : dd;

    var str = f.split('-'),
      k,
      val = [];

    for (k in str) {
      switch (str[k]) {
        case 'YY':
          val[k] = yy;
          break;
        case 'MM':
          val[k] = mm;
          break;
        case 'DD':
          val[k] = dd;
          break;
      };
    };
    return val.join('-');
  },
  unitFormat: function (num, decimal) {
      /**
       * 可用数值小于1万时，不显示单位“万”的情况。
       * decimal: 保留小数位数，-1为默认值（万，保留1位；亿保留两位；）。如果不为-1，则统一保留decimal设定的小数位数。
       */
      var dw = decimal == -1 ? 1 : decimal,
        dy = decimal == -1 ? 2 : decimal,
        val;
      var c = {
        w: '\u4E07',
        y: '\u4EBF'
      };
      if ((num / Math.pow(10, 8)).toFixed(dy) >= 1) {
        val = (num / Math.pow(10, 8)).toFixed(dy) + '<span class="unit-num">' + c.y + '</span>';
      } else if ((num / Math.pow(10, 4)).toFixed(dw) >= 1) {
        val = (num / Math.pow(10, 4)).toFixed(dw) + '<span class="unit-num">' + c.w + '</span>';
      } else {
        val = num;
      };
      return val;
  },
  boxOfficeFormat: function (num, decimal) {
      /**
       * 用于大于100且小于1000的仍以“万”为单位,显示0.1万；
       * decimal: 保留小数位数，-1为默认值（万，保留1位；亿保留两位；）。如果不为-1，则统一保留decimal设定的小数位数。
       * 
       */
      var dw = decimal == -1 ? 1 : decimal,
        dy = decimal == -1 ? 2 : decimal,
        val;
      var c = {
        w: '\u4E07',
        y: '\u4EBF'
      };
      if ((num / Math.pow(10, 8)).toFixed(dy) >= 1) {
        val = (num / Math.pow(10, 8)).toFixed(dy) + '<span class="unit-num">' + c.y + '</span>';
      } else if ((num / Math.pow(10, 4)).toFixed(dw) >= 0.1) {
        val = (num / Math.pow(10, 4)).toFixed(dw) + '<span class="unit-num">' + c.w + '</span>';
      }else if(num > 0){
        val = '<0.1' + '<span class="unit-num">' + c.w + '</span>';
      }else{
        // val = (num / Math.pow(10, 4)).toFixed(dw);
        // val = val == '0.0' ? 0 : val;
        // c.w = val == 0 ? '' : c.w;
        val = '0' + '<span class="unit-num"> </span>';
      };
      return val;
  },

  percentFormat: function (num, decimal) {
    /**
     * decimal:默认2小数，可以不传
     */
    var dec = decimal != null ? decimal : 2;
    var val = (num * 100).toFixed(dec);
    return val;
  },
  //对数据数据做快排
  dataSort: function (arr, kw) {
    function quickSort(arr) {
      if (arr.length <= 1) {
        return arr;
      };
      var num = Math.floor(arr.length / 2);
      var arrV = arr.splice(num, 1)[0];
      var numValue = arrV[kw];
      var left = [];
      var right = [];
      for (var i = 0; i < arr.length; i++) {
        if (arr[i][kw] < numValue) {
          left.push(arr[i]);
        }
        else {
          right.push(arr[i]);
        }
      }
      return quickSort(left).concat(arrV, quickSort(right));
    };
    return quickSort(arr);
  },
  //按字节截取
  byteCut: function (str, limit) {  
    var _c = str;
    var c = _c.replace(/[^\u0000-\u00ff]/g,"aa"),
      count = 0, i = 0;
    if(c.length > limit){
      for(i; i<str.length; i++){
        var _clen = str.charAt(i).replace(/[^\u0000-\u00ff]/g,"aa").length;
        if(_clen == 1){
          count ++;
        }else if(_clen == 2){
          count += 2;
        };
        if(count > limit){
          break;
        };
      };
    };
    return str.substring(0,i);
  },
  formatNumNotHtml: function (num) {
    //只用于,算万的单位数值
    if ((num / Math.pow(10, 4)).toFixed(1) >= 0.1) {
      val = (num / Math.pow(10, 4)).toFixed(1)  ;
    }else if(num > 0){
      val = 0.1;
    }else{
      val = 0;
    };
    val = Number(val);
    return val;
  }
};
exports.fmt = new formateMain();
